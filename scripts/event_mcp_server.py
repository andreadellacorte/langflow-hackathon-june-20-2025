...
class EventManager:
    ...
    def find_schedule_item(self, identifier: str) -> Optional[Dict]:
        ...
        for item in self.event_data.get('schedule', []):
            if (item.get('id', '').lower() == identifier.lower() or 
                identifier.lower() in item.get('title', '').lower()):
                return item
        return None
    
    def update_schedule_time(self, item_id: str, new_time: str, new_end_time: Optional[str] = None) -> bool:
    ...
    def add_schedule_item(self, title: str, time: str, end_time: str, location: str, 
                         description: Optional[str] = None, speaker: Optional[str] = None) -> bool:
...
class IntentClassifier:
    ...
    def __init__(self):
        ...
        self.intent_patterns = {
            'time_change': {
                'keywords': ['move', 'change time', 'reschedule', 'shift', 'delay', 'earlier', 'later', 'from', 'to'],
                'boost_patterns': [r'from \d+:\d+ to \d+:\d+', r'at \d+:\d+', r'time.*change']
            },
            'location_change': {
                'keywords': ['move to', 'change location', 'relocate', 'different room', 'new venue', 'room', 'hall'],
                'boost_patterns': [r'to room [a-z]', r'move.*to.*room', r'location.*change']
            },
            'speaker_change': {
                'keywords': ['replace speaker', 'new speaker', 'speaker change', 'substitute'],
            ...
            'query_info': {
                'keywords': ['what', 'when', 'where', 'who', 'how', 'tell me', 'info about'],
                'boost_patterns': [r'what.*\?', r'when.*\?', r'where.*\?', r'tell me about']
            }
        }
    ...
    def _extract_parameters(self, text: str, intent: str) -> Dict[str, Any]:
        ...
        if intent == 'time_change':
            ...
            # Extract event name
            event_keywords = ['keynote', 'lunch', 'panel', 'workshop', 'registration', 'coffee', 'break']
            ...
            query_keywords = ['wifi', 'password', 'parking', 'location', 'schedule', 'lunch', 'time']
    ...
    def _generate_suggested_action(self, intent: str, parameters: Dict[str, Any]) -> str:
...
async def handle_list_tools() -> list[Tool]:
    ...
    return [
        ...
        Tool(
            name="get_organizer",
            description="Get organizer contact information and event details",
            inputSchema={
                "type": "object",
                "properties": {}
            }
            ...
            if intent == 'time_change':
                ...
                if event_name and new_time:
                    ...
                    execution_result = {
                        "executed": success,
                        "action": "time_change",
                        "details": f"Updated {event_name} to {new_time}" if success else f"Failed to find {event_name}"
                    }
                ...
                if event_name and new_location:
                    ...
                    execution_result = {
                        ...
                        "details": f"Moved {event_name} to {new_location}" if success else f"Failed to find {event_name}"
                    }
